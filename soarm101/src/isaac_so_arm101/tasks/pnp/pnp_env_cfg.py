...
could not carry:
  ...
  * The bin is rebuilt from primitives at small_KLT's measured footprint instead of referencing it.
    ...
    would be cloned into every parallel environment. Second, small_KLT is only 0.059 tall at the
    scene's 0.4 scale with its floor near the bottom, so a cube resting inside it sits at about
    z = 0.02 -- under the 0.025 that object_is_lifted calls "lifted", and that same threshold gates
    object_goal_distance. Referencing the asset as-is would drive 36 of the 37 total reward weight
    to zero at the exact moment the cube lands in the bin. Nothing would error; the run would just
    never learn to finish. The rebuild keeps the footprint and raises the floor to 20 mm.
...
asset's origin is its centre, so in the authored file the bin is half sunk into the tabletop. The
same origin-is-not-the-centre trap as the table, in a place where it is easy to miss.
...
# primitives rather than referenced. small_KLT is 0.059 tall at this scale with its floor near the
# bottom, so a cube resting inside it would sit at roughly z = 0.02 -- below the 0.025 that
# object_is_lifted calls "lifted". That threshold also gates object_goal_distance, so 36 of the 37
# total reward weight would fall to zero at the exact moment the cube lands in the bin, silently.
