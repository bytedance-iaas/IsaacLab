...
from .sac_base_model import SACBaseModel


class SACActorModel(SACBaseModel):
    """SAC actor model with Tanh-squashed Gaussian output distribution.

    Inherits observation-group resolution, empirical normalization, and ``get_latent()`` from :class:`SACBaseModel`.
    ...
    .. note::
        ...
        analogous to how PPO has both ``SACBaseModel`` and ``RNNModel``. This would require adding
    ...
    def forward(
        ...
        Args:
            ...
            actions: Unused. Accepted for interface compatibility with SACBaseModel.
...
class SACCriticModel(SACBaseModel):
    ...
    from :class:`SACBaseModel`. Overrides the MLP with twin Q-networks that take concatenated
