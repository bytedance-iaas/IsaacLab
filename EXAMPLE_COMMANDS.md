...
可直接复制的命令，每条单行。算法选择、推荐并行度与已知边界见 `RELEASE_NOTES.md`。
...
| 训练规模 | `--num_envs`（推荐值见 `RELEASE_NOTES.md`）、`--max_iterations` |
| 随机种子 | `--seed 42` |
...
⚠️ `--num_envs`、`--max_iterations`、`--seed` 三项只能写成 `--` 开头的命令行参数；
写成末尾的 `路径=值` 会被静默忽略。
...
```bash
cd /workspace/isaaclab && nvidia-smi --query-gpu=memory.used --format=csv,noheader
```

显存不接近 0 表示卡上还有训练。**按进程组终止，不要只杀单个 PID** —— `pgrep` 拿到的常是外层
脚本，杀掉它之后占显存的进程仍在，下一次训练会以"显卡不可用"之类与真实原因无关的错误失败：

```bash
ps -eo pid,pgid,args | grep -E "train\.py|rsl_rl" | grep -v grep
```
...
再次用 `nvidia-smi` 确认显存回落；仍未释放则 `kill -KILL -<PGID>`。
