...
- **训练画面回传**(`--stream`,LiveKit)+ 浏览器观看页(`viewer/`),不依赖 GPU 硬件编码器
- **训练诊断报告**(`make_report.py`)、**评估导出**(ONNX)、**产物上传 TOS**(`upload_tos.py`)
- **SO-ARM101 机械臂支持**(vendored `isaac_so_arm101`:reach / lift)+ 自建的 `pnp` 翻译范例
- Helm chart 一键部署训练容器与 Sim 工作站(`docker/charts/`)

命令速查见 `EXAMPLE_COMMANDS.md`,本文只讲**怎么选**和**为什么**。
...
确认显卡空闲。**以显存为准,不要只看进程** —— Isaac Sim 的残留进程未必还叫 train.py:
...
SO-ARM101 系列(`reach` / `lift` / `pnp`,含你自己写的任务)不走核心训练脚本,用扩展包自带的入口:
...
⚠️ 用错入口报的是 `NameNotFound`(提示任务不存在),与任务代码本身无关 —— 这是最容易误判的一条。
...
训练由多个进程组成,**杀单个 PID 往往只杀掉外层脚本,占显存的那个还活着**。残留会让下一次训练以"显卡不可用""无法创建 stream"报错,与真实原因毫无关系。按进程组杀,并以显存复核。

查进程与所属进程组(第二列是 PGID):

```bash
ps -eo pid,pgid,etime,args | grep -E "train\.py|rsl_rl" | grep -v grep
```
...
复核 —— 这一步不能省,显存没回落就用 `kill -KILL -<PGID>` 再查:
...
- **操作类任务(机械臂 reach / lift / pnp)**:只有 PPO 配置。上游 Isaac Lab 和 ETH 论文都没有操作类的
...
⚠️ **推流相机是按并行环境创建的,数量等于 `--num_envs`。** 实测 16 个环境正常;4096 个环境下场景构建阶段就会长时间卡住、显卡满载、**没有任何报错**,训练根本起不来。所以推流只用来"看一眼确认场景对不对",确认完停掉,正式训练不加 `--stream`。
...
LiveKit 凭据由集群 Secret `livekit-creds` 自动注入,无需配置;若日志报 `are not set`,说明 pod 建于 Secret 之前 —— 找管理员重建 pod。

观看端是浏览器页面(本仓库 `viewer/`,随 chart 以 `viewer.enabled=true` 部署),地址与登录口令找管理员。页面会列出当前正在推流的房间,点开即看,支持多人同时观看。
...
| 加了 `--stream` 后训练卡在环境创建、无报错 | `--num_envs` 太大 —— 相机按环境创建,推流请用 16 |
| 机械臂任务报 `NameNotFound` | 用了核心训练脚本,应改用 `-m isaac_so_arm101.scripts.rsl_rl.train` |
| 训练报"显卡不可用"但没人在训 | 上一次训练有残留进程,按进程组杀干净并用 nvidia-smi 复核 |
